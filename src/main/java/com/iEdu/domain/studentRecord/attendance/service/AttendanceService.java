...
public interface AttendanceService {
    // 본인의 모든 출결 조회 [학생 권한]
    Page<AttendanceResponse> getMyAllAttendance(Pageable pageable, LoginUserDto loginUser);

    // 학생의 모든 출결 조회 [학부모/선생님 권한]
    Page<AttendanceResponse> getAllAttendance(Long studentId, Pageable pageable, LoginUserDto loginUser);

    // (학년/학기/월)로 본인 출결 조회 [학생 권한]
    Page<AttendanceResponse>  getMyFilterAttendance(Integer year, Semester semester, Integer month, Pageable pageable, LoginUserDto loginUser);

    // (학년/학기/월)로 학생 출결 조회 [학부모/선생님 권한]
    Page<AttendanceResponse>  getFilterAttendance(Long studentId, Integer year, Semester semester, Integer month, Pageable pageable, LoginUserDto loginUser);

    // 학생 출결 생성 [선생님 권한]
    void createAttendance(Long studentId, AttendanceRequest attendanceRequest, LoginUserDto loginUser);

    // 학생 출결 수정 [선생님 권한]
    void updateAttendance(Long attendanceId, AttendanceRequest attendanceRequest, LoginUserDto loginUser);
